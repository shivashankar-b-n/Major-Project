...
export default function Profile() {
  ...
  const changeLang = (l) => {
    if (l !== 'en') { toast.info('Kannada & Hindi are coming soon — the UI is already translation-ready.'); return; }
    setL(l); setLang(l);
  ...
  return (
    <div className="space-y-5 px-4 py-5">
      ...
      <div className="space-y-1.5">
        <Label className="flex items-center gap-1.5"><Globe className="h-4 w-4" /> Language</Label>
      ...
      <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
        <div className="flex items-center justify-between p-4">
          <span className="flex items-center gap-3"><Bell className="h-4.5 w-4.5 text-muted-foreground" /> <span className="text-sm font-medium">Push notifications</span></span>
          <Switch defaultChecked />
        </div>
        <div className="flex items-center justify-between p-4">
          <span className="flex items-center gap-3"><Shield className="h-4.5 w-4.5 text-muted-foreground" /> <span className="text-sm font-medium">Share location for reports</span></span>
          <Switch defaultChecked />
        </div>
        <button className="flex w-full items-center justify-between p-4 text-left transition-colors hover:bg-secondary/50">
          <span className="flex items-center gap-3"><Info className="h-4.5 w-4.5 text-muted-foreground" /> <span className="text-sm font-medium">About CivicPulse</span></span>
          <ChevronRight className="h-4 w-4 text-muted-foreground" />
        </button>
      </div>

      <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-3">
        <span className="text-xs text-muted-foreground">This is a demonstration account.</span>
